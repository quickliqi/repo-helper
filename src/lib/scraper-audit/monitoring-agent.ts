...
import { AuditReport, AuditAlert, IntegrityReport, StructuralReport, RelevanceReport, CrossCheckReport } from '../../types/scraper-audit-types';
import { supabase } from '@/integrations/supabase/client';

/**
 * Generate alerts from the individual agent reports.
 */
...
/**
 * Log the audit report to the database.
 */
export async function logAuditReport(
    ...
    try {
        ...
        // Use type assertion for the custom table since it may not be in the generated types yet
        ...
        if (error) {
            console.error('[MONITORING-AGENT] Failed to log audit report:', error);
        }
    } catch (err) {
