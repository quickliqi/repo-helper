...
import { ScrapedDeal } from '../../types/scraper-audit-types';
...
export async function runDedup(
    ...
    for (let i = 0; i < deals.length; i++) {
        ...
        // 4. Price proximity at same address (different title but same location)
        if (!result.isDuplicate) {
            const hashWithBand = `${hash}|${priceBand(price, priceVariance)}`;
            // Simple check: if same address hash exists with a very close price
            for (let j = 0; j < i; j++) {
...
/**
 * Register new deal hashes in the DB for future cross-session dedup.
 */
